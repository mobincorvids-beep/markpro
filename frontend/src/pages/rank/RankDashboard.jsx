    ...
    <DashboardLayout title="Overview">
      <div className="fade-in">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 14, marginBottom: 26 }}>
          {[
            { label: 'Projects', value: projects.length, icon: FolderOpen, color: 'var(--primary)', limit: projectsLimit, used: projectsUsed, href: '/rank/projects' },
            { label: 'Reports This Month', value: reportsUsed, icon: FileText, color: 'var(--secondary)', limit: reportsLimit, used: reportsUsed, href: '/rank/reports' },
            { label: 'Avg SEO Score', value: projects.length ? Math.round(projects.filter(p => p.latestScore).reduce((s, p) => s + p.latestScore, 0) / Math.max(1, projects.filter(p => p.latestScore).length)) || '-' : '-', icon: TrendingUp, color: 'var(--success)', href: '/rank/projects' },
            { label: 'Plan', value: user?.plan?.name || 'Free', icon: Zap, color: 'var(--warning)', href: '/billing' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <Link to="/rank/projects" className="btn btn-ghost btn-sm" style={{ fontSize: 12 }}>All <ArrowRight size={12} /></Link>
            </div>
            {projects.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '28px 0', color: 'var(--text-muted)' }}>
                ...
                <Link to="/rank/projects" className="btn btn-primary btn-sm"><Plus size={12} /> Create Project</Link>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {projects.slice(0, 5).map(p => (
                  <Link key={p._id} to={`/rank/projects/${p._id}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: 'var(--bg)', borderRadius: 8, border: '1px solid var(--border)', textDecoration: 'none' }}>
                    <div>
          ...
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <Link to="/rank/reports" className="btn btn-ghost btn-sm" style={{ fontSize: 12 }}>All <ArrowRight size={12} /></Link>
            </div>
            {reports.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '28px 0', color: 'var(--text-muted)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {reports.map(r => (
                  <Link key={r._id} to={`/rank/reports/${r._id}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: 'var(--bg)', borderRadius: 8, border: '1px solid var(--border)', textDecoration: 'none' }}>
        ...
        {!['pro', 'agency'].includes(user?.plan?.slug) && (
          <div className="card" style={{ marginTop: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(99,102,241,.07)', borderColor: 'rgba(99,102,241,.25)', flexWrap: 'wrap', gap: 12 }}>
            ...
            <Link to="/billing" className="btn btn-primary btn-sm"><Zap size={12} /> Upgrade Now</Link>
          </div>
        )}
      </div>
    </DashboardLayout>
